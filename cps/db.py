...
class Books(Base):
    ...
    DEFAULT_PUBDATE = datetime(1971, 1, 1, 0, 0, 0, 1, tzinfo=timezone.utc)  # ("0101-01-01 00:00:00+00:00")
...
class CalibreDB:
    ...
    def setup_db(cls, config_calibre_dir, app_db_path):
        cls.dispose()
        try:
            cls.engine = create_engine(os.getenv('DB_METADATA_CONNECTION'),
                                       echo=False,
                                       isolation_level="SERIALIZABLE",
                                       poolclass=StaticPool)
            ...
            log.info("Db connection error in setup_db")
            return None
        ...
        log.info("Db connection success in setup_db")
        ...
        log.info("Successed loading datatype from custom_columns")
        ...
        log.info("Successed initiating instances")
        cls._init = True
    ...
    def get_book_read_archived(self, book_id, read_column, allow_show_archived=False):
        ...
        return (bd.filter(Books.id == book_id)
                ...
                .filter(self.common_filters(allow_show_archived)).first())
    def get_book_payment(self, book_id):
        return (self.session.query(
        self.session.query(ub.Payments)
            .filter(
                ub.Payments.user_id == current_user.id,
                (ub.Payments.book_id == book_id) & (ub.Payments.is_successed == 1)
            )
            .exists()
        ).scalar())
    def get_user_is_paid(self):
        one_month_ago = datetime.now() - timedelta(days=30)
        return (self.session.query(self.session.query(ub.Payments)
            .filter(
                ub.Payments.user_id == current_user.id,
                (ub.Payments.book_id == -1) & (ub.Payments.paid_at >= one_month_ago) & (ub.Payments.is_successed == 1)
            ).exists()
        ).scalar())
    ...
    def fill_bookpage(self, database, type):
        ...
        if type == 'home':
            ...
            shelfs = (self.session.query(ub.Shelf).filter(ub.Shelf.user_id == current_user.id)).all()
            for shelf in shelfs:
                books = (self.session.query(database)
                        .filter(ub.BookShelf.shelf == shelf.id)
                        .filter(or_(database.owner.in_(owner_subquery), database.owner == 0))
                        .all())
