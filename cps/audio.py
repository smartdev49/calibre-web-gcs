...
def get_audio_file_info(tmp_file_path, original_file_extension, original_file_name, no_cover_processing):
    ...
    if original_file_extension in [".mp3", ".wav", ".aiff"]:
        ...
        title = audio_file.tags.get('\xa9nam')[0] if "\xa9nam" in audio_file.tags else None
        author = audio_file.tags.get('\xa9ART')[0] if "\xa9ART" in audio_file.tags else None
        comments = audio_file.tags.get('\xa9cmt')[0] if "pyxa9cmt" in audio_file.tags else None
        tags = audio_file.tags.get('\xa9gen')[0] if "\xa9gen" in audio_file.tags else None
        series = audio_file.tags.get('\xa9alb')[0] if "\xa9alb" in audio_file.tags else None
        series_id = str(audio_file.tags.get('trkn')[0][0]) if "trkn" in audio_file.tags else None
        publisher = ""
        pubdate = audio_file.tags.get('\xa9day')[0] if "\xa9day" in audio_file.tags else None
