...
def delete_book_file(book, calibrepath, book_format=None):
    ...
    log.error("Deleting book %s from database only, book path in database not valid: %s",
              book.id, book.path)
    # in GCS. this remove folder which contains deleting book file and cover file
    if cloud_file_helper.delete_folder(book.path):
        return True, None
...
def rename_all_files_on_change(one_book, new_path, old_path, all_new_name, gdrive=False):
    ...
    print('\n', 'rename_all_files_on_change', old_path, new_path, '\n')
    for file_format in one_book.data:
        if not gdrive:
            ...
            # BEGIN : 
            # cloud_file_helper.upload_from_filename(os.path.join(old_path, file_format.name + '.' + file_format.format.lower()),
                    # os.path.join(new_path, all_new_name + '.' + file_format.format.lower()))
            print('\n', 'rename_all_files_on_change', old_path, new_path, '\n')
            cloud_file_helper.rename_folder(old_path, new_path)
        ...
        if os.path.isdir(os.path.join(calibre_path, old_author_dir)):
            ...
            try:
                ...
                try:
                    ...
                    cloud_file_helper(old_author_dir, new_author_path)
...
def update_dir_structure_file(book_id, calibre_path, original_filepath, new_author, db_filename, type, uploaded_filename = None):
    ...
    type_dir = local_book.path.split('/')[0]
    author_dir = local_book.path.split('/')[1]
    title_dir = local_book.path.split('/')[2]
    ...
    if title_dir != new_title_dir or author_dir != new_author_dir or original_filepath:
        error = move_files_on_change(calibre_path,
                                     ...
                                     db_filename,
                                     original_filepath,
                                     path, type=type, uploaded_filename = uploaded_filename)
...
def move_files_on_change(calibre_path, new_author_dir, new_titledir, localbook, db_filename, original_filepath, path, type, uploaded_filename = None):
    new_path = os.path.join(calibre_path, new_author_dir, new_titledir)

    dir_type = localbook.path.split('/')[0]
    dir_lang = "en" # 
    dir_orig = "Originals"
    if uploaded_filename:
        dir_title = uploaded_filename.split('.')[0]
    elif db_filename:
        dir_title = db_filename.split('.')[0]
    elif new_titledir:
        dir_title = new_titledir
        
    
    dir_unicode_title = dir_title.encode('unicode-escape').decode('ascii')
    dir_author = new_author_dir
    ...
    try:
        if original_filepath:
            ...
            if type == 0:                
                ...
                full_path = os.path.join(dir_type, 
                                         dir_lang, 
                                         dir_orig, 
                                         last_prefix).replace('\\', '/')
            ...
            if type == 1:
                dir_unicode_title = new_titledir.encode('unicode-escape').decode('ascii')
                dir_hash = localbook.path.split('_')[-1]
                last_prefix = hash_unicode_utils.generate_document_string(new_titledir, new_author_dir, dir_hash)
                full_path = os.path.join(dir_type, 
                                         dir_lang, 
                                         dir_orig, 
...
def check_email(email):
    ...
    if ub.session.query(ub.User).filter(func.lower(ub.User.email) == email.lower()).first():
        ...
        raise Exception(_("Found an existing account for this Email address. Please login"))
    return email

def check_phone(phone):
    if ub.session.query(ub.User).filter(ub.User.phone == phone).first():
        log.error("Found an existing account for this Phone number")
        raise Exception(_("Found an existing account for this Phone number. Please login."))
    return phone
...
def check_birthday(birthday):
    ...
    print('\nage', age)
    if age < 13:
        raise Exception(_("User must be at least 13 years old."))
    return birthday
...
def check_password(pwd1, pwd2):
    if pwd1 != pwd2:
        raise Exception(_("Confirm Your Password"))    
    return pwd1
    

def get_user_by_email(email):
    return ub.session.query(ub.User).filter(ub.User.email == email).first()
...
def update_dir_structure(book_id,
                         ...
                         db_filename=None, type = 0, uploaded_filename = None):
    if config.config_use_google_drive:
        ...
        return update_dir_structure_file(book_id,
                                         ...
                                         original_filepath,
                                         first_author,
                                         db_filename, type=type, uploaded_filename = uploaded_filename)
...
def get_book_cover_internal(book, resolution=None):
    if book and book.has_cover:
        ...
        fname=book.path+'/'+'cover.jpg'
        try:
            cover_file = cloud_file_helper.read_as_bytes(fname)
            if cover_file:
                return send_file(io.BytesIO( cover_file ), download_name=fname, as_attachment= True)
            else:
        ...
        # this code use send_file for return cover file resoure from Local Storage
        # this block not use because cover file exist in GCS
        # if resolution:
...
def save_cover_from_filestorage(filepath, saved_filename, img):
    ...
    #### this function is to save cover file from uploaded img file #####
    # below added code upload image file to its book's directory.
    retval, msg = cloud_file_helper.upload_from_file(img, filepath + '/' + saved_filename)
    if retval:
        return True, None
    else :
        return False, _(f'Failed Upload Cover File: {msg}')
    ######## End ############
    # below not use for Google Cloud Storage File Operation
...
def save_cover(img, book_path):
    ...
    if config.config_use_google_drive:
        ...
        # this add for save cover file to GCS
        return save_cover_from_filestorage(book_path, "cover.jpg", img)
        # below block is not use for GCS. use for Local Storage
...
def do_download_file(book, book_format, client, data, headers):
    ...
    if config.config_use_google_drive:
        ...
        # below code is not use for GCS because of os.path operation use for Local Storage
        # and below code block to make book fullpath
    ...
    # BEGIN: below code make response for local storage file
