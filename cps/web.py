...
from .helper import check_valid_domain, check_email, check_username, check_phone, check_password, check_birthday, \
...
@user_login_required
def add_bookmark(book_id, book_format):    
    bookmark_key = request.form["bookmark"]
    bookmark = ub.Bookmark()
    bookmark.user_id = int(current_user.id)
    bookmark.book_id = book_id
    bookmark.bookmark_key = bookmark_key
    bookmark.format = book_format
    
    ub.session.add(bookmark)
    try:
        ub.session.commit()    
        return '{0}'.format(bookmark.id), 201
    except Exception as e:
        ub.session.rollback() 
        print('add bookmark occured : ', e)      
        return "Error", 204

@web.route("/ajax/removebookmark", methods=['POST'])
@user_login_required
def delete_bookmark():
    try:
        bookmark_id = request.form["bookmark_id"]
        ub.session.query(ub.Bookmark).filter(ub.Bookmark.id==bookmark_id).delete()
        ub.session.commit()
        return "", 201    
    except Exception as e:
        ub.session.rollback()   
        print('delete bookmark occured : ', e)      
        return "", 204
...
def render_books_list(data, sort_param, book_id, page):
    ...
    if data == "rated":
        ...
        return render_title_template('index.html', random=random, entries=entries, pagination=pagination,
                                     title=_("Books"), page=website, order=order[1], is_paid = calibre_db.get_user_is_paid())
...
def serve_book(book_id, book_format, anyname):
    ...
    if config.config_use_google_drive:
        ...
        if book_format.upper() == 'TXT':
            log.info('Serving book: %s', data.name)
            try:
                fullpath = os.path.join(config.get_book_path(), book.path, data.name + "." + book_format)
                rawdata = open(fullpath,
                               "rb").read()
    ...
    # download_bytes = cloud_file_helper.download_to_memory(download_path)
        ...
        if serve_bytes:
            ...
            response = make_response(
                ...
                # Delete For GCS
                # send_from_directory( os.path.join(config.get_book_path(), book.path), 
...
def register_post():
    ...
    nickname = strip_whitespaces(to_save.get("name", "")) if config.config_register_email else to_save.get('name')
    ...
    try:
        ...
        phonenumbers = check_phone(is_valid_phone_number( to_save.get('phone') ))
        password = valid_password(to_save.get("password"))
        birthday = check_birthday(to_save.get('birthday'))
    except Exception as ex:
    ...
    if check_valid_domain(email):
        ...
        return render_title_template('register.html', title=_("Register"), page="register")
    flash(_("Success! Please login."), category="success")
...
def register():
    ...
    return render_title_template('register.html', title=_("Register"), page="register", config=config,)
    if not config.get_mail_server_configured():
        flash(_("Oops! Email server is not configured, please contact your administrator."), category="error")
        return render_title_template('register.html', title=_("Register"), page="register")
    if feature_support['oauth']:
...
def is_valid_phone_number(phone_number):
    if bool(re.match(r'^\+\d{1,3}[-.\s]?\(?\d{1,4}\)?[-.\s]?\d{1,4}[-.\s]?\d{1,9}$', phone_number)):
        return phone_number
    raise Exception(_("Found an existing account for this Phone number"))
def send_otp(phone_number):
    formatted_number = formatted_phone_number(phone_number)
    
    verification = client.verify.services(TWILIO_VERIFY_SERVICE_ID).verifications.create(to=formatted_number, channel='sms')
    
    return formatted_number, verification.sid

def string_to_int(s):
    if s == "":
        return None  # or some other value to indicate the string was empty
    try:
        return int(s)
    except ValueError:
...
def is_valid_mobile_number(number):
    try:
        # Parse the number assuming it's in international format
        ...
        return number_type == PhoneNumberType.MOBILE or number_type == PhoneNumberType.FIXED_LINE_OR_MOBILE

    except NumberParseException:
        return False

@web.route('/otp', methods = ['POST'])
def serve_otp():
    type = int(request.form['type'])
    if type == 1: # otp send
        to_ = formatted_phone_number(request.form['phone'])
        user = ub.session.query(ub.User).filter(ub.User.phone == to_).first()
        if user is None:
            print('\nPhone number is not found. Please register.', to_)
            flash(_("Phone number is not found. Please register."))
            return jsonify({'status':'error',   'msg': "Phone number is not found. Please register."}), 400 
        
        if not is_valid_mobile_number(to_):
            print("\nPhone number is not a valid phone number. Please check your phone number")
            flash(_("Phone number is not a valid phone number. Please check your phone number."))
            return jsonify({'status':'error',   'msg': "Phone number is not a valid phone number. Please check your phone number"}), 400
        if to_:
            # send_otp
            try:
                to__, __ = send_otp(to_)
                flask_session['to'] = to__
            except Exception as e:
                flash(_("otp error : %(error)s", error=e), category="error")
                print(f"error : {str(e)}")
                return jsonify({'status':'error',   'msg':_("otp error : %(error)s", error=e)}), 400
            return jsonify({'status':'success', 'otp': __ }), 200
        else:
            # flash(_("Phone number is required"))
            return jsonify({'status':'error',   'msg':_('phone number is required')}), 400
    elif type == 2: # otp verify
        user_otp = request.form['otp']
        if string_to_int(user_otp) is None:
            return jsonify({'status':'error', "msg":"otp error"}), 400
        
        verification_check = client.verify.services(TWILIO_VERIFY_SERVICE_ID).verification_checks.create(to=flask_session['to'], code=user_otp)
        if verification_check.status == 'approved':
            # if new user register and login
            # and old user login
            user = ub.session.query(ub.User).filter(ub.User.phone == flask_session['to']).first()
            if user is None:
                flash(_("Phone number not found. Please register."))
                return jsonify({'status':'error',   'msg': "Phone number not found. Please register."}), 400
            login_user(user)
            return redirect(url_for('web.profile'))
        else:
            return jsonify({'status':'error', "msg":"verify failed"}), 400
    else:
        return jsonify({'status':'error', "msg":"type error"}), 400
...
@web.route("/pay/config")
@login_required_if_no_ano
def get_publishable_key():
    stripe_config = {"publicKey" : stripe_keys['publishable_key']}
    return jsonify(stripe_config)

@web.route("/pay/create-checkout-session", methods=['GET'])
@login_required_if_no_ano
def create_checkout_session_subscription():
    domain_url = request.root_url
    ...
    try:
        checkout_session = stripe.checkout.Session.create(
            ...
            mode="subscription",
            success_url=domain_url + "/pay/success/{CHECKOUT_SESSION_ID}",
            cancel_url=domain_url + "/pay/cancel",
        )
        payment = ub.Payments()
        payment.user_id = int(current_user.id)
        payment.book_id = -1
        payment.session_id = checkout_session.id
        
        ub.session.add(payment)
        ub.session.commit()
...
# @web.route("/pay/create-checkout-session", methods=['GET'])
@web.route("/pay/create-checkout-session2", methods=['POST'])
@login_required_if_no_ano
def create_checkout_session_customer():
    domain_url = request.root_url
    stripe.api_key = stripe_keys["secret_key"]
    
    data = request.get_json()
    book_id = data['priceId']
    book = calibre_db.session.query(db.Books).filter(db.Books.id == book_id).first()
    try:
        checkout_session = stripe.checkout.Session.create(
            payment_method_types=['card'],
            ...
            mode= 'payment',
            success_url=domain_url + "/pay/success/{CHECKOUT_SESSION_ID}",
            cancel_url=domain_url + "/pay/cancel/{CHECKOUT_SESSION_ID}",
        )
        payment = ub.Payments()
        payment.user_id = int(current_user.id)
...
@web.route('/pay/success/<string:session_id>', methods=['GET'])
@login_required_if_no_ano
def pay_success(session_id):
    print('successed:', session_id)
    paid = ub.session.query(ub.Payments).filter(ub.Payments.session_id == session_id).first()
    if paid is not None:
        paid.is_successed = 1
        ub.session.merge(paid)
        ub.session.commit()
        flash(f"Payment succeeded: {session_id}", category="info")
        return redirect(url_for("web.index"))
    else:
        ub.session.rollback()
        flash(f"Unknown session")
        return redirect(url_for("web.index"))

@web.route('/pay/cancel/<string:session_id>', methods=['GET'])
@login_required_if_no_ano
def pay_cancel(session_id):
    ub.session.query(ub.Payments).filter(ub.Payments.session_id==session_id).delete()
    ub.session.commit()
    print('canceled:', session_id)
    flash(_("Subscription canceled"), category="info")
    return redirect(url_for('web.index'))
