...
from sqlalchemy import String, Integer, SmallInteger, Boolean, DateTime, Float, JSON, Date, Text
...
class User(UserBase, Base):
    ...
    phone = Column(String(120), unique=True, default="")
    role = Column(SmallInteger, default=constants.ROLE_USER)
    password = Column(String)
    birthday = Column(Date, default=datetime.today)
...
class Payments(Base):
    ...
    user_id = Column(Integer, ForeignKey('user.id'))
    book_id = Column(Integer) # if -1 subscription else book_id
    paid_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
    is_successed = Column(Integer, default = 0)
...
def init_db_thread():
    ...
    log.info("app_DB_path in init_db_thread")
    log.info(app_DB_path)

    engine = create_engine(os.getenv('DB_METADATA_CONNECTION'), echo=False)
...
def init_db(app_db_path):
    ...
    # DELETE FOR MySQL -> Not Use Sqlite
    #
    # global app_DB_path
    ...
    # DELETE FOR MySQL -> No Use SQLite
    #
...
def get_new_session_instance():
    new_engine = create_engine(os.getenv('DB_METADATA_CONNECTION'), echo=False)
