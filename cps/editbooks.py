    ...
    try:
        if param == 'series_index':
            ...
            if handle_title_on_edit(book, vals.get('value', "")):
                rename_error = helper.update_dir_structure(book.id, config.get_book_path(), type=1)
                if not rename_error:
            ...
            rename_error = helper.update_dir_structure(book.id, config.get_book_path(), input_authors[0], type=1)
            if not rename_error:
...
def table_xchange_author_title():
    ...
    if vals:
        for val in vals:
            ...
            if edited_books_id:
                # toDo: Handle error
                edit_error = helper.update_dir_structure(edited_books_id, config.get_book_path(), input_authors[0], type=3)
            if modify_date:
                book.last_modified = datetime.now(timezone.utc)
...
def create_book_on_upload(modify_date, meta):
    ...
    if meta.is_audio:
        path = os.path.join("Audiobooks/"+author_dir, title_dir).replace('\\', '/')
    else:
        path = os.path.join("Ebooks/"+author_dir, title_dir).replace('\\', '/')

    # # combine path for cloud storage
    try:
    ...
    db_book = db.Books(title, "", sort_authors, datetime.now(timezone.utc), pubdate.astimezone(timezone.utc),
...
# if edit_role is true, point after upload come here
...
def upload_book_formats(requested_files, book, book_id, no_cover=True):
    ...
    for requested_file in requested_files:
        ...
        if current_filename != '':
            ...
            file_name = ''.join(current_filename.rsplit('.', 1)[0:-1])
            # Use For GCS
            saved_filename = os.path.join(book.path, file_name + '.' + file_ext).replace('\\', '/')
            # END
            ...
            # Add For GCS
            file_size = cloud_file_helper.file_size(saved_filename)
            # END
